...
void createLocalC(Matrix &C,mesh m){
    ...
    C.at(0).at(0) += -3.0*A/2;  C.at(0).at(1) += 3.0*A/2;
    C.at(1).at(0) += -3.0*A/2;  C.at(1).at(1) += 3.0*A/2;
}

void createLocalG(Matrix &G,mesh m){
    float Y = m.getParameter(Y);
    G.at(0).at(0) += -Y/3;  G.at(0).at(1) += Y/3;
    G.at(1).at(0) += -Y/3;  G.at(1).at(1) += Y/3;
}

void createLocalI(Matrix &I,mesh m){
    float D = m.getParameter(D);
    I.at(0).at(0) += -D/2.0;  I.at(0).at(1) += D/2.0;
    I.at(1).at(0) += -D/2.0;  I.at(1).at(1) += D/2.0;
}
...
Vector createLocalb(int element,mesh &m){
    ...
    float W = m.getParameter(W), N = m.getParameter(N), l = m.getParameter(ELEMENT_LENGTH);
